...
class AppointmentController extends Controller
{
    ...
    public function store(Request $request, $doctorId)
    {
        ...
        if ($appointment->fee > 0) {
            return redirect()->route('appointments.payment', $appointment)
                ->with('success', 'Rendez-vous réservé ! Veuillez procéder au paiement.');
        }
    ...
    public function confirm(Appointment $appointment)
    {
        ...
        if (!$user->isDoctor() || !$appointment->doctor || $appointment->doctor->user_id !== $user->id) {
            \Log::warning('Accès non autorisé', [
                'user_id' => $user->id,
                'doctor_user_id' => $appointment->doctor ? $appointment->doctor->user_id : null,
                'is_doctor' => $user->isDoctor(),
            ]);
            abort(403, 'Accès non autorisé');
        }
        ...
        $patientUser->notify(new \App\Notifications\AppointmentConfirmedNotification($appointment));
    ...
    /**
     * Show the payment form for the appointment.
     */
    public function showPayment(Appointment $appointment)
    {
        ...
        if ($appointment->payment_status === 'paid' || $appointment->fee <= 0) {
            return redirect()->route('appointments.show', $appointment)
                ->with('info', 'Ce rendez-vous est déjà payé ou gratuit.');
        }
    ...
    /**
     * Process the payment for the appointment.
     */
    public function processPayment(Request $request, Appointment $appointment)
    {
        ...
        $request->validate([
            'payment_method' => 'required|in:card,flouci,bank_transfer',
            ...
            'phone_number' => 'required_if:payment_method,flouci|nullable|string',
        ]);
        ...
        try {
            ...
            return redirect()->route('appointments.show', $appointment)
                ->with('success', 'Paiement effectué avec succès ! Votre rendez-vous est confirmé.');
