...
class AuthController extends Controller
{
    ...
    public function registerPatient(Request $request)
    {
        ...
        return redirect('/dashboard')->with('success', 'Compte patient créé avec succès !');
    }
    ...
    public function registerDoctor(Request $request)
    {
        $request->validate([
            ...
            'new_specialty' => 'nullable|string|max:255',
        ...
        // Handle specialty creation if 'other' is selected
        $specialtyId = $request->specialty_id;
        if ($request->specialty_id === 'other' && $request->new_specialty) {
            $specialty = \App\Models\Specialty::create([
                'name' => $request->new_specialty,
                'is_active' => true,
            ]);
            $specialtyId = $specialty->id;
        }

        // Create user - temporarily exclude location fields until database is updated
        $userData = [
            'first_name' => $request->first_name,
            'last_name' => $request->last_name,
            'email' => $request->email,
        ...
        // Add location fields if they exist in the database
        try {
            $user = User::create($userData);
        } catch (\Illuminate\Database\QueryException $e) {
            if (str_contains($e->getMessage(), 'Unknown column')) {
                // Fallback: create user without location fields
                $user = User::create([
                    'first_name' => $request->first_name,
                    'last_name' => $request->last_name,
                    'email' => $request->email,
        ...
        $doctor = \App\Models\Doctor::create([
            ...
            'clinic_address' => $request->clinic_address ?? $request->address,
            'clinic_phone' => $request->clinic_phone ?? $request->phone_country_code . $request->phone,
        ...
        return redirect('/dashboard')->with('success', 'Inscription réussie ! Votre compte médecin a été créé avec succès.');
    }
