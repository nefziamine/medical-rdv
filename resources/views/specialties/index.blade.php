    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center py-4">
        ...
        <div class="hidden md:flex items-center space-x-8">
          ...
          <a href="/specialties" class="text-green-600 font-semibold relative">
            Spécialités
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center animate-slide-up">
        <h1 class="text-5xl lg:text-6xl font-extrabold text-white mb-6">
          Nos <span class="bg-gradient-to-r from-emerald-200 to-white bg-clip-text text-transparent">Spécialités</span>
        </h1>
        <p class="text-xl text-white/90 max-w-2xl mx-auto leading-relaxed">
          Découvrez notre expertise médicale regroupée pour vous simplifier la vie.
        </p>
      </div>
    </div>
  </section>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($specialties as $specialty)
          <div class="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 border border-gray-100 p-6 group">
            <div class="flex items-center mb-4">
              <div class="w-16 h-16 bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl flex items-center justify-center mr-4 group-hover:scale-110 transition-transform duration-300">
                <i class="fas fa-stethoscope text-white text-2xl"></i>
              </div>
              <div>
                <h3 class="text-xl font-bold text-gray-900 mb-1">{{ $specialty->name }}</h3>
                <p class="text-gray-600 text-sm">{{ $specialty->subtitle ?? '' }}</p>
              </div>
            </div>
            <p class="text-gray-600 mb-6 leading-relaxed">{{ $specialty->description ?? '' }}</p>
            <a href="{{ route('specialties.show', $specialty->slug) }}" class="inline-flex items-center space-x-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 transform hover:scale-105">
