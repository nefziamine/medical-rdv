...
Route::get('/', function () {
    $specialties = collect(); // Start with empty collection
    try {
        if (\DB::connection()->getPdo()) {
            $specialties = Specialty::active()->ordered()->limit(8)->get();
        }
    } catch (\Exception $e) {
...
Route::middleware('auth')->group(function () {
    ...
    // Payment routes
    Route::get('/appointments/{appointment}/payment', [AppointmentController::class, 'showPayment'])->name('appointments.payment');
